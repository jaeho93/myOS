...
//		if(sb_inode_used_checker(fp,inode_number)==1)
//			return;
		show_inode(inode_number,fp);
	}
	else if(strcmp("myshowblock",ptr[0])==0){
		printf("i am myshowblock ptr[1]\n");
	}
...
int sb_inode_used_checker(FILE *fp,int inode_num)//myshowinode하기 전에 사용여부 확인해서 없으면 애초에 다음 함수 안나오게 stop시키기
{
	fseek(fp,16+inode_num-1,0);
	if(getc(fp)!='0' || getc(fp)!='1')
