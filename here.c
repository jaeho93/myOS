...
void set_inode_address(){
		...
		if(size==1){
				...
				for(int k=0; k< size-1 ;k++){
						...
						fseek(fp,16+512+1024+79*512 + 128*8*single_num + 10*k,0);
				...
				if( ((size-103 )/ 102) == 0 )   //num : double이 할당해야하는 데이터블록개수
						num = ((size-103)/102);
				else num =( (size-103) /102) +1;
				...
				for(int i=0; i< num ;i++){
						...
						if(i != num-1)
								last = 102;
						else last = size - 103 - (102 * num);   //double에서 필요한 마지막
