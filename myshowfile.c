	...
	//cnt는 single내에서 쓰이는 direct의 갯수입니다
{
	int basic=102;
	if(cnt != 0)
		basic=cnt;//mycat할때는 102개 다쓰지만 myshowfile할때 경우에 따라 써야하는 갯수가 다르기 때문에 쓰는 것임
	int direct[102];
...
void mycat(FILE *fp,LINK head,char*file_name)
{
	...
	if(strcmp(k->name,file_name)==0){
		get_inode(fp,ip,16+512+1024+79*(k->inode_num-1));//inode의 시작위치까지만 보내주면 아이노드 정보 10진화 해서 ip에 다 리턴해줌
