...
LINK dir_to_list(FILE *);//fp가 fs에서 데이터블록의 위치를 포인트 하고 있어서, 데이터를 읽어들임과 동시에,  노드상에서 sib=NULL 일때까지 다음 노드를 가리켜 linked_list형태를 유지
void print_time(int);//생성시간을 inode * ip를 통해 저장할 수 있음
void bin_to_str(FILE *fp);//2진법화 된 문자열을 텍스트 형식으로 변환
int binary_changer(FILE *fp, int size);//2진법을 10진법으로 변환
void print_list(LINK head);//print ls
void print_list_i(LINK);//print ls -i
void print_list_l(LINK, FILE*);//print ls -l
void get_inode(FILE *fp, inode * ip, int num);//해당 커서에서 2진법으로 저장된 문자열을 텍스트로 binary_changer()를 통해 변환한다음 해당 아이노드의 정보를 노드에 저장
void link_free(LINK);//하위노드들 전부 free처리 해줌
void print_inode_info(int num, FILE *fp);//get_inode()에서 준 inode의 정보를 출력함
void prompt(LINK head, FILE * fp);//mother_inode와 h2(자식)이 한칸씩 가면서 mother_inode=1(root)가 될때까지 /넣으면서  한 문자열에 저장해줌
char * find_my_name(LINK head, int myinode);//myinode와 일치할때까지 재귀함수 써서 head=head->next 반복한후 맞으면 해당 아이노드와 관련된 이름 리턴
void mypwd(LINK head, FILE * fp);//pwd
void myrmdir(LINK head,FILE *fp);//rmdir 인데 일부 수정 필요
void rm_target_dir_from_list(FILE *fp,LINK head,char*rmdir_str);//fs가서 지우고자 하는 디렉토리와 관련된 정보 삭제
...
void mymkdir(LINK head, FILE *fp);//fs에서 sb부분 아이노드,db부분 모두 처리
void write_to_inode_dir(FILE *fp, int sb_inode_empty, int sb_data_empty);//fs에서 아이노드부분 000000~ 으로 초기화
void write_to_data_dir(FILE *fp, int sb_inode_empty, int sb_data_empty);//fs에서 db부분 0000~ 으로 초기화
void link_newdir_to_list(FILE * fp, LINK head, int sb_inode_empty, char newdir_str[]);
LINK dir_to_list_sib(FILE *fp, int sb_inode_empty);//디렉토리 만들고 sib를 만드는데 fname[]에 디렉의 이름을 넣기 때문에 결국 자기 자신을 포인트하게 되서 null 효과를 볼수 있음
void binary(int time, char*str, int num);//time=inode_number, empty_string, file_size=10 --> 10진수를 2진법으로 저장하는데 int형이라 +'0' 하여 문자열화
void write_name_to_dir(FILE * fp, LINK head, int sb_inode_empty, char *newdir_str);//디렉토리의 이름을 fs상에 기입
void name_to_bin(char * fromname, char res[][9]);//텍스트를 2진법으로 바꾸는 건데 해당 프로그램에서는 무쓸모
